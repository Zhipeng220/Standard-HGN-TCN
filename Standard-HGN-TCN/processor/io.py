...
class IO():
    ...
    def init_environment(self):
        ...
        if self.arg.use_gpu:
            ...
            self.gpus = list(range(len(gpus)))  # Remap to 0, 1, 2...

            # ✅ Try MPS (Metal Performance Shaders) for Mac M-series chips
            if torch.backends.mps.is_available():
                self.dev = "mps"
                print('=' * 60)
                print('Using MPS (Metal Performance Shaders) - Mac GPU')
                print('=' * 60)
                # For MPS, we don't use multiple devices
                self.gpus = [0]
            # Try CUDA
            elif torch.cuda.is_available():
                self.dev = "cuda:0"
                print('=' * 60)
                print(f'Using CUDA GPU: {gpus}, mapped to: {self.gpus}')
                print('=' * 60)
            # Fallback to CPU
            else:
                self.dev = "cpu"
    ...
    def gpu(self):
        ...
        # model parallel
        # ✅ MPS doesn't support DataParallel, only use for CUDA
        if self.arg.use_gpu and len(self.gpus) > 1 and self.dev.startswith('cuda'):
            self.model = nn.DataParallel(self.model, device_ids=self.gpus)
            print(f'Using DataParallel on {len(self.gpus)} GPUs')
